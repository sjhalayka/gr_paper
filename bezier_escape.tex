...
In this paper we will describe a numerical solution for the deflection of light and neutrinos by the Sun (e.g. where $v \sim c$), and for the relativistic orbit of Mercury around the Sun (e.g. where $v \ll c$).

No tensor calculus is involved -- only high school physics is required.
For instance, the light clock thought experiment is required in order to understand what kinematic time dilation.
After one understands the kinematic time dilation, it is not too far a stretch for them to understand graviational time dilation -- gravitation is the interruption of internal process by another process at some distance away.
...
We will use Euler integration to keep things simple.
...
\item
Eq. \ref{eq_position} goes to show that the planet Mercury dwells more when it's closer to the Sun (e.g. where $0 \leq \beta < 1$), causing the rest of the relativistic perihelion precession.
...
\bibitem{einstein} Relativity -- The Special and General Theory. (1916)
